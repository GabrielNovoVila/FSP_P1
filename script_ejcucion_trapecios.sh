...
divisiones_values=(1000 10000 100000 1000000 2000000 5000000)
...
mpicc -Wall -o trapecios trapecios.c -lm

# Bucle principal
for division in "${divisiones_values[@]}"; do
    for np in $(seq $min_np $max_np); do
        for version in 0 1 2; do
            ...
            for ((i=1; i<=repeticiones; i++)); do
                ...
                mpirun -np "$np" trapecios "$version" "$division"
            done
        done
    done
done
